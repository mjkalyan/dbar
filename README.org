...
/dbar/ can be used statically by retrieving its output via pipe (=dbar | foo=), command expansion (=$(dbar)=), etc.
*** Examples
...
#+end_src
This works exactly as we want, hooray! Also note we took advantage of being in a shell environment to put information about the currently running song into the /dbar/ window title.
